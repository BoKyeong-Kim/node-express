...
- 브라우저가 요청 헤더 형태로 서버에 정보를 보내는 것과 마찬가지로, 서버 역시 브라우저가 렌더링하거나 표시하지 않은 정보를 보냄
- 응답 헤더에 들어있는 정보는 일반적으로 메타 데이터와 서버 정보

⇒ 브라우저는 콘텐츠를 렌더링할 때 **Content-Type**의 정보를 사용해 렌더링 방법을 결정

헤더에는 Content-Type 외에도 응답이 압축 됐는지 여부, 사용하는 인코딩 종류 같은 정보도 들어있음

- 응답 헤더에는 서버에 관한 정보가 포함될 때도 많음
- 서버타입이 무엇인지, 때로는 운영체제가 무엇인지까지도 들어있기도 함
- 익스프레스에서는 헤더를 비활성화할 수 있음 → X-Powered-By
...
- Content-Type 헤더는 중요
    - 이 정보가 없으면 클라이언트는 콘텐츠 렌더링 방법을 스스로 추론해야함
...
- 응답의 바디가 실제 콘텐츠인 것과 마찬가지로, 요청에는 요청 헤더 외의 바디도 들어있음
- 일반적인 GET 요청에는 바디가 없지만 POST 요청에는 보통 바디가 들어있음



### 요청 객체

- 요청객체는 핵심 노드 객체인 http.IncomingMessage의 인스턴스로 시작
- 이름은 원하는 대로 쓸 수 있지만 보통 req와 request를 가장 많이 사용



### 응답 객체

- 응답 객체는 노드의 핵심 객체인 http.ServerResponse의 인스턴스
- 보통 res, resp, response를 사용



### 콘텐츠 렌더링

- 콘텐츠를 렌더링할 때는 뷰를 레이아웃 안에 렌더링하는 res.render를 가장 자주 사용
    - 테스트 페이지를 빨리 만들어보고 싶을 때는 res.send
    - res.query로 쿼리스트링의 값을 가져옴



### 폼 처리

- 폼을 처리할 때 그 정보는 보통 req.body에 들어있음(가끔 req.query에 들어있기도 함)
- req.xhr을 써서 요청이 에이잭스 요청인지 브라우저 요청인지 판단
...
- API를 제공할 때 폼 처리와 비슷하게 매개변수는 보통 req.query에 들어있지만 req.body도 사용할 수 있음
    - API에서 다른 점은 보통 JSON이나 XML, 평문을 반환하며 일반적으로는 잘 쓰지 않는 PUT, POST, DELETE 같은 HTTP 매서드를 자주 사용한다는 점

(end point라는 용어는 API에서 함수를 가리킬 때 자주 사용)
...
- 서버 사이드 템플릿은 리액트나 앵귤러, 뷰 등의 프론트엔드에 의해 빠르게 대체되고 있지만 HTML 이메일을 보내는 등 아직 프론트엔드에서는 사용할 수 없는 영역도 있음
- 템플릿이란?
    - 한 언어에서 다른 언어를 만들어내는 것

⇒ 템플릿 구조를 잘 갖추면 개발 시간이 줄어들고, 웹사이트에 일관성이 생기며 변덕스러운 레이아웃이 어딘가에 숨어있는 경우도 줄어들게된다.
...
→ {{today}}에 들어갈 값만 제공하면 되는 구조



### 핸들바 기초

핸들바 : 또 다른 템플릿 엔진인 머스태시를 확장해서 만들어짐

- 프론트엔드와 백엔드 양쪽에서 자바스크립트와 쉽게 통합할 수 있음
- 익숙한 문법을 사용할 수 있음

템플릿을 이해하기 위해서는 **콘텍스트**의 개념을 이해해야함

템플릿을 렌더링할 때는 템플릿 엔진에 **콘텍스트 객체**를 전달하며, 이를 통해 템플릿에 데이터를 삽입

`{ name : 'Buttercup' }` 라는 콘텍스트 객체 존재

`<p>Hello, {{ name }}! </p>` 라는 템플릿

→ 템플릿 엔진은 {{ name }} 을 `Buttercup` 로 바꾼다.

*(템플릿에 HTML을 넣고싶으면 중괄호 3개를 사용 {{{ }}})*

→ 주석은 {{ !절대 비밀인 주석  }}, < !--알려져도 상관없는 주석 -->으로 표기

- 절대 비밀인 주석은 브라우저에 전송되지않음
- 알려져도 상관없는 주석은 사용자가 HTML 소스를 통해 볼 수 있음



### 서버 사이드 템플릿

- HTML을 완성한 다음에 클라이언트에 전송
- 클라이언트 사이드 템플릿에서는 사용자가 HTML소스 보기를 통해 템플릿을 볼 수 있지만, 서버 사이드 템플릿을 사용하면 사용자가 템플릿이나 콘텍스트 객체를 볼 수 없음
- 서버사이드 템플릿은 프로그램 구성 정보를 숨기는 효과 외에도 **템플릿** 캐싱이 가능해지므로 성능을 올릴 수 있음
...
- 뷰 : 웹사이트에 존재하는 개별 페이지
    - 기본적으로 익스프레스는 뷰를 views 서브디렉터리에서 찾음
- 레이아웃 : 특별한 뷰 → 템플릿의 템플릿
    - 사이트에 존재하는 페이지 거의 대부분이 비슷한 형태로 이뤄져있으므로 레이아웃음 중요
    - 코드를 매 페이지마다 반복하지 않기 위해 레이아웃 사용

⇒ 뷰가 가장 먼저 렌더링 되고, 레이아웃이 렌더링 되는 순서



### 익스프레스에서 레이아웃 사용

- 대부분의 페이지가 같은 레이아웃을 사용할 때가 많으니, 뷰를 렌더링할 때마다 레이아웃을 지정하려면 불편
    - 뷰 엔진을 생성할 때 기본 레이아웃의 이름을 지정함
...
→ 익스프레스는 기본적으로 views 서브디렉터리에서 뷰를 검색하고 views/layouts에서 레이아웃을 검색